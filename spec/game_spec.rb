...
RSpec.describe Codebreaker::Game do
  ...
  context 'instance variables after creating Game object' do
    ...
    context 'attempts' do
      it 'sets easy lvl for attempts' do
        expect(subject.attempts).to eq(Codebreaker::Game::DIFFICULTIES.dig(:easy, :attempts))
      end

      it 'sets medium lvl for attempts' do
        expect(Codebreaker::Game.new(:medium).attempts).to eq(Codebreaker::Game::DIFFICULTIES.dig(:medium, :attempts))
      end

      it 'sets hard lvl for attempts' do
        expect(Codebreaker::Game.new(:hard).attempts).to eq(Codebreaker::Game::DIFFICULTIES.dig(:hard, :attempts))
      end
    end

    context 'hints' do
      it 'sets easy lvl for hints' do
        expect(subject.hints.size).to eq(Codebreaker::Game::DIFFICULTIES.dig(:easy, :hints))
      end
  ...
  context '#equal_codes?' do
    ...
    it 'has to return false' do
      expect(subject.equal_codes?('1111')).to be_falsey
    end

    it 'has to return true' do
      expect(subject.equal_codes?('1234')).to be_truthy
    end
  end
  ...
  context '#handle_guess' do
    ...
    it 'calls handle_numbers' do
      expect(subject).to receive(:handle_numbers).and_call_original
  ...
  context '#handle_numbers' do
    it 'calls check_numbers_for_correct_position method' do
      ...
      expect(subject).to receive(:check_numbers_for_correct_position).and_call_original
